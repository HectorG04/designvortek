...
export default async function OrdersPage() {
  ...
  const inProgressCount = allOrders.filter(
    (o) =>
      o.status === 'quoted' || o.status === 'accepted' || o.status === 'in_progress',
  ).length

  const startOfMonth = new Date(new Date().setDate(1)).setHours(0, 0, 0, 0)
  const deliveredThisMonth = allOrders.filter(
    (o) =>
      o.status === 'delivered' && new Date(o.created_at).getTime() >= startOfMonth,
  )
  ...
  const subtitle = `${allOrders.length} ${
    allOrders.length === 1 ? 'commission' : 'commissions'
  ...
  const filterCounts: Record<string, number> = {
    ...
    closed: allOrders.filter((o) => o.status === 'closed' || o.status === 'cancelled')
      .length,
  }
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        />
        <MiniStat label="In progress" value={inProgressCount} icon={Briefcase} />
        <MiniStat
          label="Delivered this month"
          value={deliveredCount}
          icon={CheckCircle2}
        />
        <MiniStat label="Total revenue" value={fmtMoney(totalRevenue)} icon={DollarSign} />
      </div>

      {/* Filter chips (visual only — render All as active) */}
      <div
        role="group"
        ...
        {FILTERS.map((f) => {
          ...
          return (
            <span
              ...
              className={
                active
                  ? 'inline-flex items-center gap-2 rounded-full border border-burgundy-700 bg-burgundy-700 text-cream-50 px-3.5 py-1.5 text-[0.8125rem]'
                  : 'inline-flex items-center gap-2 rounded-full border border-border-light bg-parchment-50 text-ink-700 hover:bg-parchment-200 px-3.5 py-1.5 text-[0.8125rem] cursor-default transition-colors'
              }
            >
              {f.label}
              <span
                className={
                  active
                    ? 'min-w-[20px] text-center text-[0.6875rem] font-semibold px-1.5 py-0.5 rounded-full bg-cream-50/20 text-cream-50'
                    : 'min-w-[20px] text-center text-[0.6875rem] font-semibold px-1.5 py-0.5 rounded-full bg-parchment-300 text-ink-700'
                }
              >
        ...
        {allOrders.length === 0 ? (
          <div className="px-5 py-16 text-center text-ink-500">
            ...
            <p className="mt-1 text-sm">
              New commission submissions will appear here as customers fill the order form.
            </p>
            <Link
              href="/admin"
              className="inline-flex items-center gap-1 mt-5 text-[0.6875rem] font-semibold uppercase tracking-[0.15em] text-burgundy-700 hover:text-burgundy-500"
            >
                ...
                {allOrders.map((order) => (
                  ...
                  >
                    <td className="px-5 py-4">
                      ...
                      <div className="text-xs text-ink-500 mt-0.5 truncate max-w-[240px]">
...
function Th({
  ...
  return (
    <th
      className={`px-5 py-3 text-left text-[0.6875rem] font-semibold uppercase tracking-[0.15em] text-ink-500 whitespace-nowrap ${
        className ?? ''
      }`}
    >
      {children}
...
function MiniStat({
  ...
  return (
    <div className="relative bg-parchment-100 border border-border-light rounded-xl px-5 py-4">
      {showDot && (
        <span
          aria-label="Needs attention"
          className="absolute top-3 right-3 w-2 h-2 rounded-full bg-burgundy-700"
        >
      ...
      <div className="font-display text-2xl md:text-[1.875rem] font-semibold text-ink-900 leading-none tracking-[-0.015em]">
        {value}
      </div>
    </div>
  )
