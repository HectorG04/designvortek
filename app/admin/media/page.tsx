    ...
    >
      ...
      <div className="mb-6 flex items-start gap-3 bg-gold-100 border border-gold-300 rounded-xl px-4 py-3">
        ...
        <div className="text-[0.875rem] text-ink-700">
          ...
          <p className="mt-1">
            For now, host images on Supabase Storage or a CDN and paste direct URLs into the portfolio and blog editors.
          </p>
        </div>
      </div>
        ...
        {FILTERS.map((f) => {
          ...
          return (
            <span
              ...
              className={
                isActive
                  ? 'inline-flex items-center gap-2 rounded-full border border-burgundy-700 bg-burgundy-700 text-cream-50 px-3.5 py-1.5 text-[0.75rem] font-semibold tracking-[0.05em] cursor-default'
                  : 'inline-flex items-center gap-2 rounded-full border border-border-light bg-parchment-100 text-ink-700 px-3.5 py-1.5 text-[0.75rem] font-semibold tracking-[0.05em] cursor-default opacity-70'
              }
            >
              {f.label}
              <span
      ...
      <section className="bg-parchment-100 border border-border-light rounded-xl p-5">
        <header className="mb-4 flex items-center gap-2">
          <ImageIcon size={16} strokeWidth={1.8} className="text-gold-700" />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {PLACEHOLDER_GRADIENTS.map((bg, i) => (
            <div
              key={i}
              className="rounded-lg overflow-hidden border border-border-light bg-parchment-50 flex flex-col"
            >
              <div className="aspect-square w-full relative" style={{ background: bg }}>
              ...
              <div className="px-3 py-2.5 flex items-center justify-between gap-2">
                <span className="text-[0.75rem] font-mono text-ink-700 truncate">placeholder-{(i + 1).toString().padStart(2, '0')}.webp</span>
                <span className="text-[0.6875rem] text-ink-400 font-mono flex-shrink-0">—</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <p className="text-[0.75rem] text-ink-500 mt-5 text-center">
          These tiles are visual placeholders. Real uploads land here once storage is wired up.
        </p>
      </section>
    </AdminShell>
  )
