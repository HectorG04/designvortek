...
import {
  ChevronLeft, Mail, Phone, ShoppingBag, DollarSign, Calendar,
...
export async function generateMetadata({ params }: CustomerDetailProps): Promise<Metadata> {
...
export default async function CustomerDetailPage({ params }: CustomerDetailProps) {
  ...
  const totalSpent = orders.reduce((s, o) => s + Number(o.quoted_price ?? 0), 0)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5 lg:gap-6">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <Stat label="Lifetime spend" value={fmtMoney(totalSpent)} icon={DollarSign} />
            <Stat label="Average order" value={fmtMoney(avgOrder)} icon={CheckCircle2} />
          </div>
          ...
          <section className="bg-parchment-100 border border-border-light rounded-xl overflow-hidden">
            <header className="px-5 py-4 border-b border-border-light flex items-center justify-between">
              <h2 className="font-display text-lg font-semibold text-ink-900 tracking-tight">Orders</h2>
                  ...
                  {orders.map((o) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5 text-sm text-ink-700">
                        {o.service_type}
                        {o.style && <span className="text-ink-500"> · {o.style}</span>}
                      </td>
                      <td className="px-5 py-3.5 text-[0.8125rem] font-mono text-ink-700">
                        {o.quoted_price != null ? fmtMoney(Number(o.quoted_price)) : <span className="text-ink-400">—</span>}
                      </td>
                      <td className="px-5 py-3.5"><StatusPill status={o.status} /></td>
                      <td className="px-5 py-3.5 text-[0.8125rem] text-ink-500 font-mono">{fmtAgo(o.created_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <section className="bg-parchment-100 border border-border-light rounded-xl p-5">
            <header className="mb-4">
              <h2 className="font-display text-lg font-semibold text-ink-900 tracking-tight">Activity</h2>
              <p className="text-[0.75rem] text-ink-500 mt-0.5">Order events for this customer.</p>
            </header>
            <ol className="flex flex-col gap-3">
              {orders.map((o) => (
                <li key={`act-${o.id}`} className="flex gap-3">
                  <span className="mt-1 w-7 h-7 rounded-full flex items-center justify-center bg-burgundy-100 text-burgundy-700 flex-shrink-0">
                    <FileText size={14} strokeWidth={1.8} />
                  </span>
                  <div className="min-w-0 flex-1">
                    <div className="text-[0.875rem] text-ink-900">
                      Submitted{' '}
                      <Link href={`/admin/orders/${o.id}`} className="text-burgundy-700 hover:text-burgundy-500 transition-colors font-semibold">
                        order #{o.id}
                      </Link>{' '}
                      — {o.service_type}
                      {o.style && <span className="text-ink-500"> · {o.style}</span>}
                    </div>
                    <div className="text-[0.75rem] text-ink-500 mt-0.5 font-mono">
                      {fmtAbsolute(o.created_at)} · status: <StatusPill status={o.status} className="ml-1" />
                    </div>
                  </div>
                </li>
              ))}
          ...
          <section className="bg-parchment-100 border border-border-light rounded-xl p-5">
            <header className="mb-3 flex items-center justify-between">
              <h2 className="font-display text-lg font-semibold text-ink-900 tracking-tight">Internal notes</h2>
              <span className="text-[0.6875rem] uppercase tracking-[0.12em] text-ink-500">Per-customer notes coming in M5</span>
            </header>
            <textarea
              disabled
              placeholder="Private notes about this customer will be persisted here once the customer profile schema lands…"
              className="w-full min-h-[120px] bg-parchment-50 border border-border-light rounded-md p-3 text-sm text-ink-700 placeholder:text-ink-400 resize-y disabled:cursor-not-allowed disabled:opacity-70"
            />
            <p className="text-[0.75rem] text-ink-500 mt-2">
              For now, jot notes per-order in each order&apos;s internal notes field.
            </p>
          </section>
        </div>
        ...
        <aside className="flex flex-col gap-5">
          <section className="bg-parchment-100 border border-border-light rounded-xl p-5">
            <div className="flex flex-col items-center gap-3 pb-4 border-b border-border-light">
              ...
              <div className="text-center">
                <div className="font-display text-xl font-semibold text-ink-900 leading-tight">{customerName}</div>
                <div className="text-[0.75rem] text-ink-500 mt-1">Customer since {customerSince}</div>
              </div>
            </div>

            <dl className="flex flex-col divide-y divide-border-light">
              <Row icon={Mail} label="Email">
                <a href={`mailto:${customerEmail}`} className="font-mono text-[0.8125rem] text-ink-700 hover:text-burgundy-700 transition-colors break-all">
                  {customerEmail}
                </a>
              </Row>
              <Row icon={Phone} label="Phone">
                {customerPhone ? (
                  <a href={`tel:${customerPhone}`} className="font-mono text-[0.8125rem] text-ink-700 hover:text-burgundy-700 transition-colors">
                    {customerPhone}
                  </a>
                ) : (
              ...
              <Row icon={Calendar} label="First order">
                <span className="text-[0.8125rem] text-ink-700 font-mono">{fmtAbsolute(firstOrder.created_at)}</span>
              </Row>
              <Row icon={MessageSquare} label="Last activity">
                <span className="text-[0.8125rem] text-ink-700 font-mono">{lastActivity}</span>
              </Row>
            </dl>
          </section>
          ...
          <section className="bg-parchment-100 border border-border-light rounded-xl p-5">
            ...
            <dl className="flex flex-col gap-3">
              <KeyValue label="Total orders" value={String(totalOrders)} />
              <KeyValue label="Lifetime spent" value={fmtMoney(totalSpent)} accent />
...
function Row({
  ...
  return (
    <div className="flex items-start gap-3 py-3">
      <Icon size={14} strokeWidth={1.8} className="text-ink-400 mt-0.5 flex-shrink-0" />
      <div className="flex flex-col min-w-0 flex-1">
        <span className="text-[0.6875rem] uppercase tracking-[0.12em] text-ink-500 mb-0.5">{label}</span>
        {children}
      </div>
    </div>
  )
      ...
      <span
        className={
          accent
            ...
            : 'text-[0.8125rem] font-mono text-ink-900'
        }
      >
        {value}
      </span>
