/* =====================================================================
   PORTFOLIO PIECES — Supabase-backed with in-memory fallback.
   ...
   Data flow:
     1. Async helpers (fetchAllPieces, fetchPieceBySlug, ...) try Supabase
        ...
        bypasses RLS so we read both published and unpublished if needed —
        but the public fetchers filter to is_published = true).
     2. If Supabase is empty (table not seeded yet) or errors, helpers fall
        back to PORTFOLIO_PIECES below — the snapshot data that shipped
        with this file.
...
export interface PortfolioPiece {
  ...
  delivered: string
  tags: string[]
  /** Tailwind gradient (used as fallback backdrop behind transparent edges
   *  of the hero image, and on masonry cards before the image paints). */
  gradient: string
  /** Aspect ratio for the masonry card. 4/5 standard, 3/2 wide/group,
   *  3/4 slightly taller, 1/1 token. */
  aspect: '4/5' | '3/4' | '3/2' | '1/1'
  /** True = show on homepage portfolio strip. All start true; flip to
   *  false to hide from strip (still appears in /portfolio masonry). */
  featured: boolean
  heroImage: string
...
/* ---------- The data ---------- */
export const PORTFOLIO_PIECES: Record<string, PortfolioPiece> = {
  'vesper-goldclaw-feral-wizard': {
    ...
    artistNote: "The signet ring and gold talons came from Tess's brief. They ended up doing more work than the armor did.",
  },
  'aderall': {
    slug: 'aderall',
...
/* =====================================================================
   SYNC HELPERS — always return the in-memory snapshot.
...
export function getAllPieces(): PortfolioPiece[] {
  return Object.values(PORTFOLIO_PIECES).sort(
    (a, b) => new Date(b.delivered).getTime() - new Date(a.delivered).getTime(),
  )
}

/** Featured-only pieces (for the homepage strip). Pass a limit to cap it. */
export function getFeaturedPieces(limit?: number): PortfolioPiece[] {
...
export function getCategoryCounts(): Record<string, number> {
  ...
  for (const piece of Object.values(PORTFOLIO_PIECES)) {
    counts.all += 1
